...
// Normal distribution player: targets a specific average equity loss per turn.
// Weights moves using a normal distribution centered at (bestEquity - meanLoss)
// with the given standard deviation. This produces natural-feeling imperfect play.
class NormalPlayer : public ComputerPlayer
{
public:
	NormalPlayer(double meanLoss = 10.0, double stdDev = 6.0, const UVString &name = MARK_UV("Intermediate"));
	virtual ~NormalPlayer();

	virtual bool isUserVisible() const { return true; }
	virtual ComputerPlayer *clone() { return new NormalPlayer(m_meanLoss, m_stdDev, m_name); }

	virtual Move move();
	virtual MoveList moves(int nmoves);

	double meanLoss() const { return m_meanLoss; }
